...
class NoVisualization(object):
    ...
    def __init__(self, seq_info , count_human_last):
        self.count_human = count_human_last
        self.frame_idx = seq_info["min_frame_idx"]
        self.last_idx = seq_info["max_frame_idx"]
        self.pbar = tqdm(total=self.last_idx)
        # self.pbar = tqdm(total=self.last_idx - self.frame_idx +1)  
...
class Visualization(object):
    ...
    def draw_groundtruth(self, track_ids, boxes, points):
        ...
        for track_id, box in zip(track_ids, boxes):
            ...
            self.viewer.rectangle(*box.astype(np.int), label=track_id)
